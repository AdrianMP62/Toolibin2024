...
ggplot(mds1.I.pts, aes(x=MDS1, y=MDS2, colour = Wetland, pch = Survey)) + xlim(Xmin,Xmax) + ylim(Ymin,Ymax) + geom_point(size=5) + coord_fixed(ratio = 1) + theme(text = element_text(size=15)) + scale_colour_manual(values=c("#F8766D", "#00BA38"))
...
ggplot(mds2.I.pts, aes(x=MDS1, y=MDS2, colour = Wetland, pch = Survey)) + xlim(Xmin,Xmax) + ylim(Ymin,Ymax) + geom_point(size=5) + coord_fixed(ratio = 1) + theme(text = element_text(size=15)) + scale_colour_manual(values=c("#C77CFF","#F8766D","#00BA38"))
...
plot_ly(color = ~factor(wetland), colors = c("#00BA38","#F8766D","#C77CFF"), showlegend = T) %>%
  add_markers(data = mds3.I.pts, x = ~MDS1B, y = ~MDS2B, z = ~MDS3B) %>%
  add_paths(data = m, x = ~MDS1B, y = ~MDS2B, z = ~MDS3B)
```

```{r}
rawdata.I4 <- rawdata.I.orig[, 2:37] #remove species names
rawdata.I4 <- t(rawdata.I4)
row.names(rawdata.I4) <- gsub("[.]", "-", row.names(rawdata.I4))
Inv.fact.orig <- read.csv("./Newdata/Invert factors.csv", row.names = 1) #import site factor variables
Inv.fact4 <- Inv.fact.orig
#rawdata.I[is.na(rawdata.I)]<-0 # replace na with 0
```


##Cluster analysis of invertebrate data with referenbce sites
...
```{r}
#cluster analysis
inv.dist <- vegdist(rawdata.I4, method = "bray")
inv.clust <- hclust(inv.dist, method = "average")
plot(inv.clust, cex=0.75)
```

```{r}
#using dendextend to produce a dendrogram
dend <- as.dendrogram(inv.clust)
dend %>% set("labels_col", value = c("black","black","black","black","black","black","red","red","red","black","black","black","black","black","black","black","black","black","black","black","black","black","black","black","black","black","black","black","black","black","black","black","black","red","red","red"), k=36) %>% set("labels_cex", 0.75) %>% plot
```
